...
检索 Benchmark 使用固定的 100 条样例、统一 `Top-K=5` 和人工标注的相关 chunk。以下为本地 `bge-m3` 真实实验结果；小型 `test_project` 上 Hybrid 的整体召回与排序质量最佳，Reranker 并未在该数据集上带来提升，因此不作“Rerank 必然更好”的泛化结论。

| 策略 | Hit@5 | Recall@5 | MRR | NDCG@5 | 平均延迟 |
| --- | ---: | ---: | ---: | ---: | ---: |
| BM25 | 98.00% | 88.25% | 81.23% | 78.55% | 1.00 ms |
| Dense Vector | 95.00% | 84.75% | 78.57% | 75.51% | 2455.94 ms |
| **Hybrid** | **99.00%** | **89.92%** | **83.82%** | **81.23%** | 2402.45 ms |
| Hybrid + Rerank | 94.00% | 84.33% | 67.15% | 67.75% | 6029.10 ms |
| Multi-Query + Rerank | 94.00% | 84.50% | 66.93% | 67.62% | 10533.33 ms |

Agent 侧提供 50 条任务基准集，覆盖符号定位、源码读取、项目结构、文档问答、代码流程、测试执行与安全拒答。它与检索 Benchmark 分开统计，避免把“检索命中”误当成“Agent 任务成功”。

当前使用本地 `qwen3.5:4b`、真实 Tool Calling 与 `bge-m3` 检索运行的报告见 [agent_eval_report_qwen35_50_round3_after_key_badcase_fix.json](outputs/experiments/agent_eval_report_qwen35_50_round3_after_key_badcase_fix.json)：

| 任务数 | 任务成功率 | 首工具准确率 | 工具 F1 | 禁止工具安全率 | 答案关键词覆盖率 | 平均延迟 | P95 延迟 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 50 | 86.00% | 88.00% | 81.33% | 100.00% | 97.00% | 20.75 s | 66.26 s |

该报告保留每个任务的工具调用、答案、Trace 与失败原因。评测结果受当前任务集、模型版本和本地硬件环境影响，不将其泛化为通用 Agent 能力结论。

针对真实评测中发现的高价值问题，项目维护了 8 条人工 Bad Case 回归集，最近一次真实模型回归为 8/8 通过：

| 回归范围 | 任务成功率 | 首工具准确率 | 工具 F1 | 禁止工具安全率 | 答案关键词覆盖率 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 显式源码读取、文档+入口、符号调用链、结构问答、安全拒答 | 100.00% | 100.00% | 95.83% | 100.00% | 100.00% |

报告见 [agent_bad_case_regression_report_qwen35_round8.json](outputs/experiments/agent_bad_case_regression_report_qwen35_round8.json)。
